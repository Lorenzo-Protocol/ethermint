...
func startInProcess(cfg Config, val *Validator) error {
	...
	tmNode, err := node.NewNode(
		...
		servercmtlog.CometLoggerWrapper{Logger: val.Ctx.Logger.With("module", val.Moniker)},
	)
	if err != nil {
		return err
	}
	...
	if val.AppConfig.JSONRPC.Enable && val.AppConfig.JSONRPC.Address != "" {
		...
		val.jsonrpc, _, err = server.StartJSONRPC(val.Ctx, val.ClientCtx, tmRPCAddr, tmEndpoint, val.AppConfig, nil)
		if err != nil {
			return err
		}
...
// FreeTCPAddr Get a free address for a test CometBFT server
